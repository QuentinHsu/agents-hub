...
struct AgentLogo: View {
    ...
    init(provider: ProviderKind, size: CGFloat = 24, isEnabled: Bool = true, fallbackColor: Color = .secondary) {
        self.provider = provider
        self.label = provider.displayName
        self.fallbackSystemImage = provider.symbolName
        self.size = size
    ...
    var body: some View {
        Group {
            if let document = AgentLogoLoader.document(named: provider.logoName) {
                logoView(document)
            } else {
